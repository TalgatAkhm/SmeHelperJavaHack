...
public class WorkerFetcher {
    ...
    public static List<User> fromAvito(final int avitoId) {
        String baseURL = "https://rest-app.net/api/ads?login=mikel1998@mail.ru&token=cea83367c2231a09e8dce54b70c2dc91&category_id=" + String.valueOf(avitoId) + "&limit=3";
    ...
    public static List<User> fromUs(final User user) {
        List<User> users = new ArrayList<>();
        try {
            users = clientApiPost.getWorkers(user.getName()).execute().body();
            if (users != null) {
